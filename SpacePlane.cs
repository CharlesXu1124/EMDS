...
namespace Blimp_GCS
{
    ...
    class SpacePlane
    {
        ...
        public SpacePlane(Double lat, Double lon, Double s, Double h)
        {
            ...
            speed = s;
            heading = h;
            using (var plane_image = Blimp_GCS.Properties.Resources.UAV)
            {
        ...
        public bool TargetReached(Double refLat, Double refLon)
        {
            if (Math.Pow(latitude - refLat, 2) + Math.Pow(longitude - refLon, 2) < 3)
            {
                return true;
            } else
            {
